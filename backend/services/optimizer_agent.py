...
class OptimizerAgent:
    ...
    def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'FROM' in query_upper and ',' in query:
                from_clause = query_upper.split('FROM')[1].split('WHERE')[0] if 'WHERE' in query_upper else query_upper.split('FROM')[1]
    ...
    def suggest_indexes(self, query: str, schema_info: Dict = None) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.error(f"Error suggesting indexes: {str(e)}")
            return []
    ...
    def interpret_query_plan(self, query_plan: List[Dict]) -> Dict[str, Any]:
        ...
        if high_cost_ops:
            interpretation['summary'] = f"Query has {len(high_cost_ops)} high-cost operation(s). "
        else:
    ...
    def comprehensive_optimization(
        ...
        try:
            ...
            logger.info("Step 1: Analyzing original query...")
            analysis = self.analyze_query(original_query)
            
            # Step 2: Get query plan interpretation
            logger.info("Step 2: Interpreting query execution plan...")
            query_plan = db_connection.get_query_plan(original_query)
            plan_interpretation = self.interpret_query_plan(
                query_plan.get('plan', []) if query_plan['success'] else []
            )
            
            # Step 3: Generate optimized query using LLM
            logger.info("Step 3: Generating optimized query...")
            ...
            logger.info("Step 4: Generating index recommendations...")
                ...
                'index_recommendations': [
                    {
                        ...
                        'column': idx['column'],
                        'reason': idx['reason'],
                        'sql_command': f"CREATE INDEX idx_{idx['table']}_{idx['column']} ON {idx['table']}({idx['column']});"
                    }
    ...
    def _estimate_improvement(self, analysis: Dict, optimizations: List[str]) -> str:
        ...
        if improvement_score >= 50:
            return f"Estimated 40-60% improvement"
        elif improvement_score >= 30:
            return f"Estimated 20-40% improvement"
        elif improvement_score >= 15:
            return f"Estimated 10-20% improvement"
        else:
            return f"Minor optimization (< 10% improvement)"
